...
import {Full, EditForm} from '@heytea/heyyo'
...
class App extends Component<{ Auth: IAuth, UI: IUI } & RouteComponentProps> {
  submit = async () => {
    const {Auth: {login, referrer, setReferrer, loginForm: {name, password}}, history: {replace}} = this.props;
    const loginData = await login();
    if (loginData[code] === 302001) {
      Auth.setForm({name: 'passwordInit', valObj: {name, oldPassword: password}})
      replace('/passwordInit')
    } else if (loginData[code] === codeSuccess ) {
      
      setTimeout(() => replace(referrer || ''), 100);
      setReferrer('')
    }
  }
